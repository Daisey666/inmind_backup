...
class DLLEXPORT cConfigManager {
  ...
  public:
    ...
    //tingyao
    //add for stream
    void setptrDouble(float *ptr){
        dataptr=ptr;
    }
    float* getptrDouble(){ return dataptr; }
    void setStreamLength(int ln){ streamln=ln; }
    int getStreamLength(){ return streamln; }

    //initial in main, push in csvSink, getoutputptr and getcurrent in main
    void initialOutput(int ln){ 
        dataoutput=new float[ln];
        outputln=ln;
        currentpos=0;
    }
    void resetCurrentPos(){ currentpos=0; }
    void setOutputPtr(float *fdata, size_t datasize){
        memcpy(dataoutput,fdata,datasize);
    }
    void pushdataOutput(float *fdata, int datanum){
        for(int i=0;i<datanum;i++)
            dataoutput[currentpos+i]=fdata[i];
        currentpos+=datanum;
    }
    void setOutputLength(int ln){ outputln=ln; }
    int getOutputLength(){  return outputln; }
    float* getOutputPtr(){ return dataoutput; }
